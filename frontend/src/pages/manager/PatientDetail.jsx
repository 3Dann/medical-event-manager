  ...
  return (
        ...
        {customNodes.length > 0 && (
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-xs text-slate-600 flex-shrink-0">
              {completedCount}/{customNodes.length} {t('completed_nodes')}{activeCount > 0 ? ` · ${activeCount} ${t('active_nodes')}` : ''}
            </span>
          </div>
        )}
                  ...
                  <div className={`rounded-xl p-3 ${isFixed ? `ring-1 ${s.ring} ${s.bg}` : 'border border-slate-100 bg-white hover:bg-slate-50'} transition-colors`}>
                    ...
                    {isEditing && (
                      <div className="mt-3 pt-3 border-t border-slate-200 space-y-2">
                        ...
                        <div className="flex gap-2 justify-end">
                          <button type="button" onClick={() => setEditingNode(null)} className="btn-secondary text-xs py-2 px-3">{t('common:cancel', { ns: 'common' })}</button>
                          <button type="button" onClick={() => saveEditNode(node)} className="btn-primary text-xs py-2 px-3">{t('common:save', { ns: 'common' })}</button>
                        </div>
                      </div>
                    )}
                  </div>
    ...
    {showJourneyModal && (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[88vh] flex flex-col" dir="rtl">
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
            <div>
              <h3 className="font-bold text-slate-800 text-lg">{t('journey_templates')}</h3>
              <p className="text-xs text-slate-500 mt-0.5">{t('journey_templates_hint')}</p>
            </div>
                ...
                <div key={cat}>
                  ...
                  <div className="space-y-1">
                    {tpls.map(tpl => {
                      ...
                      const isSuggested = !isApplied && patientConditionTags.includes(tpl.key)
                      const isSelected  = selectedTplPreview?.key === tpl.key
                      return (
                        <button key={tpl.key}
                          onClick={() => setSelectedTplPreview(tpl)}
                          className={`w-full text-right px-3 py-2.5 rounded-xl transition-colors flex items-center gap-2.5
                            ${isSelected ? 'bg-violet-50 border border-violet-200' : 'border border-transparent hover:bg-slate-50'}`}
                        >
              ...
              {!selectedTplPreview ? (
                ...
                <div>
                  ...
                  <div className="space-y-3 mb-6">
                    {selectedTplPreview.nodes.map((node, i) => (
                  ...
                  {appliedTemplateKeys.has(selectedTplPreview.key) ? (
                    ...
                    <button
                      onClick={() => applyTemplate(selectedTplPreview.key)}
    ...
    <div className="mt-2">
      {items.length > 0 && (
        <div className="space-y-1 mb-1">
          ...
          {items.length > 0 && (
            <p className="text-[10px] text-slate-600 mt-1">{done}/{items.length} {t('completed_nodes')}</p>
          )}
        </div>
      )}
