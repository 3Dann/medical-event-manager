...
function PatientRequestsPanel({ patientId }) {
  ...
  const { toast: reqToast, showToast: showReqToast, dismissToast: dismissReqToast } = useToast()
    ...
    <div className="border border-slate-200 rounded-2xl overflow-hidden mt-6">
      ...
      {open && (
        <div className="divide-y divide-slate-100">
          {loading ? (
            <div className="py-6 text-center text-slate-600 text-sm">{t('common:loading', { ns: 'common' })}</div>
          ) : requests.length === 0 ? (
            <div className="py-8 text-center text-slate-600 text-sm">{t('no_requests')}</div>
          ) : (
            requests.map(r => (
...
// TYPES built inside MeetingForm to support i18n
const TYPES_STATIC = [
          ...
          {hasActions && (
            <div>
              ...
              {meeting.action_items.map((item, i) => (
                <div key={i} className={`text-sm flex items-center gap-2 ${item.done ? 'line-through text-slate-500' : 'text-slate-700'}`}>
                  <span>{item.done ? '✅' : '☐'}</span>
                  <span>{item.task}</span>
                  {item.responsible && <span className="text-xs text-slate-600">({item.responsible})</span>}
                </div>
