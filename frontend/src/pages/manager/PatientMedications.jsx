...
export default function PatientMedications() {
  ...
  const checkInBackground = () => {
    checkCtrlRef.current?.abort()
    checkCtrlRef.current = new AbortController()
    setCheckingInteractions(true)
    fetchAll(checkCtrlRef.current.signal)
  }
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 space-y-3">
          ...
          {candidates.length > 0 && (
            <div className="space-y-2 border-t border-blue-200 pt-3">
              ...
              {candidates.map((c) => (
                <div key={c.name} className="flex items-center justify-between bg-white rounded-xl border border-blue-200 px-3 py-2">
          ...
          {!extracting && candidates.length === 0 && extractDocId && (
            <p className="text-xs text-slate-600">{t('extract_prompt')}</p>
          )}
        </div>
      )}
