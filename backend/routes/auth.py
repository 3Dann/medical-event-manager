...
def register(request: Request, user_data: UserCreate, db: Session = Depends(get_db),
    ...
    # ── ת"ז + טלפון — ולידציה ובדיקת כפילות ──────────────────────────────────
    # id_number and phone are EncryptedText — SQL equality comparison is not possible
    # (Fernet is non-deterministic). Load all pending rows once and check both in Python.
    clean_id = None
    ...
    # ── יצירת הבקשה ──────────────────────────────────────────────────────────
    # hashed_password = placeholder — הסיסמה האמיתית נוצרת ב-approve_registration בלבד
...
def e2e_login(request: Request, data: E2ELoginRequest, db: Session = Depends(get_db)):
    ...
    def _audit(action: str, status: int, user_id=None, user_name=None):
        """Write to audit trail without ever breaking the auth flow."""
        try:
            db.add(models.UserActivityLog(
                user_id=user_id, user_name=user_name,
                action_type=action, ip_address=client_ip,
                user_agent=user_agent, status_code=status,
            ))
            db.commit()
        except Exception:
            logger.exception("E2E audit write failed (action=%s) — auth continues", action)
            db.rollback()
    ...
    if not e2e_secret or data.e2e_secret != e2e_secret:
        ...
        _audit("e2e_login_failed", 401)
        raise HTTPException(status_code=401, detail="Unauthorized")

    user = db.query(models.User).filter(models.User.email == data.email).first()
    if not user:
        logger.warning("E2E login rejected: user not found %s from %s", data.email, client_ip)
        _audit("e2e_login_user_not_found", 404)
        raise HTTPException(status_code=404, detail="User not found")

    _audit("e2e_login_success", 200, user_id=user.id, user_name=user.full_name)
...
def forgot_password_verify(request: Request, data: ForgotPasswordVerifyRequest, db: Session = Depends(get_db)):
    ...
    user = db.query(models.User).filter(sqlfunc.lower(models.User.email) == email_key).first()
    if not user:
        raise HTTPException(status_code=400, detail="לא נמצאה בקשת אימות פעילה — ודא שהאימייל נכון וחזור לשלב הראשון")
    ...
    if datetime.now(tz.utc) > expires:
        raise HTTPException(status_code=400, detail="פג תוקף הזיהוי (10 דקות) — חזור לשלב הראשון ובקש קוד חדש")
