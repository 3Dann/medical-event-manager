...
### עדיפות עליונה — מניתוח פערים SRS Brief
- [ ] **טופס אינטייק מורחב** — שדות דמוגרפיה (מין, גיל, מצב משפחתי, BMI, כתובת), ADL/IADL/Mini Mental Test, מטרות מובנות (15+ אופציות), רשימת תרופות עם מינון, חתימות דיגיטליות (ויתור סודיות, ייפוי כוח), ניתוח פערים אוטומטי בסיום
- [ ] **פורטל מטופל** — כניסה עצמאית, ציר זמן read-only, מסמכים, סטטוס תביעות, אפשרות שליחת בקשות
- [ ] **ניהול משימות חוצה-תיקים** — תצוגת "היום שלי" לכל המלווים, תעדוף קלנדרי, חיבור ליומן Google Calendar
- [ ] **דשבורד ניהולי** — סקירת כל המלווים, עומס תיקים, סטטוסים, פערים, התראות אסקלציה אוטומטית
...
- [ ] **מסעות לפי אינדיקציה** — 10-15 מחלות שכיחות עם צמתי קבלת החלטות, רופאים, בדיקות, וידע קליני מובנה לפי מחלה
- [ ] **העשרת מאגר רופאים** — שדות חסרים: זמני קבלה, WhatsApp, תשלום ביקור פרטי, נגישות, אמפתיה, בהירות הסבר, שיתוף עם OrMed, כוכבית קשר פעיל
- [ ] **פורטל ברוקר/סוכן** — תפקיד ייעודי, מבט על מטופלים שהפנה, סטטוס תביעות, התחשבנות
- [ ] **דוחות אוטומטיים** — דוח אחרי אינטייק, דוח חודשי, דוח למשפחה, דוח סיום התקשרות

### עדיפות בינונית
- [ ] **דוחות וייצוא** (FR-REP) — דוח סיכום מטופל PDF/Excel, גרף ציר זמן
- [ ] **SMTP אמיתי** — כרגע 2FA ותזכורות ללא שליחת mail אמיתית
- [ ] **התראות פערי ביטוח** — זיהוי אוטומטי של כיסוי חסר (סיעוד, אובדן כושר עבודה)
- [ ] **מאגר מכונים ומעבדות** — MRI, CT, אולטרסאונד, בדיקות דם פרטי, פתולוגיה, שיקום

### עדיפות נמוכה
- [ ] **מחקרים קליניים** — רשימה עדכנית ופתוחה לפי אינדיקציה
- [ ] **מודל עסקי/חיוב** — דמי מנוי MCM, עמלת ברוקר, ניהול חיוב מרכזי, מניעת עקיפה
- [ ] **תפקידי ניהול נוספים** — מנהל רפואי / משפטי / פיננסי עם פיקוח ייעודי
